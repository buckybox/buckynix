...
generateCalendar deliveries visibleWindow =
    let
        ...
        days =
            Dict.union deliveryDays paddingDays |> Dict.values
    in
        buildCalendar days
...
calendarDay day maxCount =
    let
        ...
        height =
            (maxBarHeight - minHeight) * toFloat (count) / toFloat (maxCount) + minHeight

        dow =
            toString (Date.dayOfWeek day.date) |> String.left 1
    in
        Collage.group
...
controlView model =
    let
        ( selectedFrom, selectedTo ) =
            ( model.selectedWindow.from, model.selectedWindow.to )

        ( visibleFrom, _ ) =
            ( model.visibleWindow.from, model.visibleWindow.to )

        fromOffset =
            DateExtra.diffDays visibleFrom selectedFrom

        toOffset =
            DateExtra.diffDays visibleFrom selectedTo

        iconWidth =
            5

        fromPosition =
            fromOffset * barWidthWithMargin + iconWidth - barMargin

        toPosition =
            (toOffset + 1) * barWidthWithMargin - iconWidth - barMargin
...
deliveryView model =
    let
        rows =
            case model.selectedDeliveries of
                [] ->
                    [ tr
                        [ class "text-center" ]
